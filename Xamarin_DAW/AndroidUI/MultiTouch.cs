...
/*
REVISION:
the multi-Touch API would work as follows

1. we interpret all input devices as "multi-touch" or "emulated multi-touch"

2.a. when the input device is a mouse

we treat a "touch down" event as either the primary or secondary button being pressed

primary and secondary button clicks will be detectable

hovering state will be ignored

2.b. when the input device is a trackpad

we detect finger down/up via native platform-host multi-touch api

we assume finger input 2.c.

2.b. when the input is a pen

we assume it is a finger input 2.c.

2.c. when the input is regular fingers

we take the first finger down as the primary input and track its location as the mouse cursor

we take all further finger down's as secondary inputs and track their positions as well

3. we compute the delta's of each pointer relative to the primary pointer's touch-down event location

3.a. when input is touchpad we compute delta based on trackpad touch event normalized positions since trackpad input does not map to screen coordinate
...
struct Pointer {
    ...
    // false if we do not support location
    bool hasLocation;
};
struct Touch {
    Pointer * pointers;
    // mice only have 1 pointer
    bool isPrimaryMouseButton;
    bool isSecondaryMouseButton;
};
*/
...
namespace Xamarin_DAW.AndroidUI
{
    ...
    public class Touch
    {
        ...
        public class Data : ICloneable
        {
            ...
            internal PointF MouseToPointF() => new PointF(location.x, location.y);
            internal PointF NormalizedToPointF() => new PointF(normalized_location_on_input_surface.y, normalized_location_on_input_surface.y);
